...
#   pymupdf                   PDF typography — font & size per line (STEP 3)
...
#   broken table fragments, photo captions).
#   build_xml.py rebuilds that hierarchy.
#
#   The key idea: the structure is already in the PDF — as
#   TYPOGRAPHY. The editor set subsection headings in GothamBold
#   11pt and body text in Whitney 9pt. So we do not guess from the
#   words; we read what the document already declares:
#
#       Gotham-Black     16pt   Goal title
#       GothamBold       11pt   subsection heading   <- take
#       Whitney-*         9pt   body paragraph       <- take
#       Whitney-Semibold 8.5pt  photo caption        <- drop
#       Whitney-Semibold   ~8pt chart title          <- drop
#       Whitney-Medium    ~6pt  chart axis labels    <- drop
#       Gotham-Black       8pt  running header       <- drop
#
#   Chart labels, captions and typesetting notes are set in other
#   faces, so they fall out on their own — no keyword blocklist.
#
...
#
#   One <subsection> = one chunk, as a rule. The heading and body
#   become searchable text; the Goal number, name, pages, and
#   heading become metadata — the provenance that rides along.
#
#   One constraint decides where we may cut: the encoder reads at
#   most 256 TOKENS. Anything past that is silently dropped before
#   the vector is made, so an over-long chunk would be indexed by
#   its opening lines alone. We therefore cap a chunk at 172 words
#   (heading included) and split at PARAGRAPH boundaries — and if a
#   single paragraph is still too long, at SENTENCE boundaries.
#   Sentences are never cut.
#
#   Split chunks carry a "part" field. Their heading, goal and
#   pages are unchanged, so provenance survives the split.
#
...
#
#   Three things about Qwen3 shape the wrapper below:
#     ① it is causal, not seq2seq → AutoModelForCausalLM
#     ② it is a chat model → wrap input in the chat template
